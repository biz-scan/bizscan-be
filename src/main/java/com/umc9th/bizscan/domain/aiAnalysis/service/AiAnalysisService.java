...
public class AiAnalysisService {
  ...
  public AnalysisRequestResponse analyzeStore(Long storeId) {
    ...
    Store store =
      ...
      analysisRepository.findByStore(store).ifPresent(existingAnalysis -> {
          // 해당 분석의 요청 상태 확인
          AnalysisRequest existingRequest = analysisRequestRepository.findByAnalysis(existingAnalysis)
                  .orElse(null);
          ...
          if (existingRequest != null) {
              ...
              if (existingRequest.getStatus() == AnalysisStatus.FAILED) {
                   ...
                   * @OnDelete(action = OnDeleteAction.CASCADE)는 DB 레벨에서 연관된 테이블들을 삭제함
                   * delete(existingRequest) 없이 delete(existingAnalysis)로 Analysis 연쇄 삭제 동작 시
                   * existingRequest는 영속성 컨텍스트에 Managed 상태로 살아있고,
                   * existingAnalysis는 Removed 상태로 삭제 예정이 됨
                   * 이 상태에서 flush() 호출 시 살아있는(Persistent) existingRequest가 삭제 예정인 existingAnalysis를 참조하고 있어서 문제가 됨
                   * 따라서 JPA 레벨에서 명시적으로 existingRequest를 삭제해야함.
