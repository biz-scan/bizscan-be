...
public class ActionNoteController {
    ...
    @Operation(
            ...
            description = "특정 실행 전략(ActionPlan)을 바탕으로 실행 노트에 등록합니다."
    )
    @ApiErrorCodeExamples({ErrorCode.ACTION_PLAN_NOT_FOUND, ErrorCode.ACTION_NOTE_ALREADY_EXISTS})
    @PostMapping()
    public ApiResponse<ActionNoteResDTO.AddDTO> addActionNote(@RequestBody ActionNoteReqDTO.AddDTO addDTO) {
    ...
    public ApiResponse<ActionNoteResDTO.DeleteDTO> deleteActionNote(
            @Parameter(description = "실행 전략 ID", example = "1")
    ...
    public ApiResponse<List<ActionNoteResDTO.ActionNotesDTO>> getActionNotes(
            ...
            @Parameter(description = "완료 여부 (true: 완료된 노트, false: 진행 중인 노트)", example = "false") @RequestParam Boolean isCompleted) {
        return ApiResponse.onSuccess(SuccessCode.OK, actionNoteService.getActionNotes(storeId, isCompleted));
    }

    @Operation(
            summary = "실행 노트 상세 조회 API",
            description = "특정 실행 전략(ActionPlan)의 내용과 포함된 상세 실행 전략(ActionDetail) 리스트, 진행도를 조회합니다. " +
    ...
    public ApiResponse<ActionNoteResDTO.ActionNoteDTO> getActionNote(
            @Parameter(description = "조회할 실행 전략 ID", example = "1")
            @PathVariable Long actionPlanId) {
    ...
    public ApiResponse<ActionNoteResDTO.UpdateActionDetailDTO> updateActionDetail(
        ...
        return ApiResponse.onSuccess(SuccessCode.OK, actionNoteService.updateActionDetail(actionDetailId, isCompleted));
    }
